...
`;
  ...
  // whatsapp_camera_agent.js
  if (!frida_objc_bridge_default.available) {
    send({ type: "agent-error", message: "\u5F53\u524D\u8FDB\u7A0B\u4E0D\u652F\u6301 Objective-C Runtime" });
  } else {
    let getGlobalExport = function(name) {
      if (typeof Module.getGlobalExportByName === "function") {
        return Module.getGlobalExportByName(name);
      }
      return Module.getExportByName(null, name);
    }, nsString = function(value) {
      return frida_objc_bridge_default.classes.NSString.stringWithUTF8String_(Memory.allocUtf8String(value));
    }, rect = function(x, y, width, height) {
      return [[x, y], [width, height]];
    }, transform = function(scale, tx, ty) {
      return [scale, 0, 0, scale, tx, ty];
    }, clearPreparedImages = function() {
      preparedImages.forEach(function(image) {
        image.release();
      });
      preparedImages.clear();
    }, releaseQrCodeObject = function() {
      if (qrCodeObject !== null) {
        qrCodeObject.release();
        qrCodeObject = null;
      }
    }, rememberCameraController = function(object) {
      if (!object || !object.$className || object.$className.indexOf("WACameraController") === -1 || activeCameraController !== null && activeCameraController.handle.equals(object.handle)) return;
      ...
      activeMetadataOutput = output && !output.isNull() ? new frida_objc_bridge_default.Object(output).retain() : null;
      activeMetadataConnection = connection && !connection.isNull() ? new frida_objc_bridge_default.Object(connection).retain() : null;
    }, releaseMetadataPipeline = function() {
    ...
    }, releaseMetadataPlaceholder = function() {
      if (metadataPlaceholder !== null) {
        metadataPlaceholder.release();
        metadataPlaceholder = null;
      }
    }, findMetadataOutput = function(controller) {
      try {
        const ivars = controller.$ivars;
        const names = Object.keys(ivars);
        for (let index = 0; index < names.length; index += 1) {
          const value = ivars[names[index]];
          if (value && value.$className && value.isKindOfClass_(frida_objc_bridge_default.classes.AVCaptureMetadataOutput)) return value;
        }
        for (let index = 0; index < names.length; index += 1) {
          const value = ivars[names[index]];
          if (!value || !value.$className || !value.isKindOfClass_(frida_objc_bridge_default.classes.AVCaptureSession)) continue;
          const outputs = value.outputs();
          for (let outputIndex = 0; outputIndex < outputs.count(); outputIndex += 1) {
            const output = outputs.objectAtIndex_(outputIndex);
            if (output.isKindOfClass_(frida_objc_bridge_default.classes.AVCaptureMetadataOutput)) return output;
          }
        }
      } catch (error) {
      ...
      const data = frida_objc_bridge_default.classes.NSData.alloc().initWithBase64EncodedString_options_(
        nsString(payload.qrDataBase64),
        0
      );
      if (!data || data.length() === 0) throw new Error("\u4E8C\u7EF4\u7801\u539F\u59CB\u6570\u636E\u89E3\u7801\u5931\u8D25");
      const parsedCode = frida_objc_bridge_default.classes.WAQRCodeDataParser.parseFromData_symbolVersion_(data, symbolVersion);
      const dataLength = Number(data.length());
      const message = nsString(payload.qrText);
      const code2 = parsedCode || message;
      const object = frida_objc_bridge_default.classes.WACameraQRCodeObject.alloc().init();
      object.setTime_(CACurrentMediaTime());
      object.setBounds_(rect(0.1, 0.1, 0.8, 0.8));
      object.setCode_(code2);
      ...
      return {
        ...
        dataLength,
        codeClass: code2.$className
      };
    }, stopDeliveryTimer = function() {
      if (deliveryTimer !== null) {
        clearInterval(deliveryTimer);
        deliveryTimer = null;
      }
    }, deliverQrCode = function() {
      ...
      frida_objc_bridge_default.schedule(frida_objc_bridge_default.mainQueue, function() {
        try {
          ...
          controller.captureOutput_didOutputMetadataObjects_fromConnection_(
            output || NULL,
            frida_objc_bridge_default.classes.NSArray.arrayWithObject_(metadataPlaceholder),
            connection
          );
          send({
    ...
    }, startDeliveryTimer = function() {
      stopDeliveryTimer();
      deliverQrCode();
      if (!delivered) deliveryTimer = setInterval(deliverQrCode, 250);
    }, prepareImage = function(width, height) {
      ...
      const white = frida_objc_bridge_default.classes.CIImage.imageWithColor_(frida_objc_bridge_default.classes.CIColor.colorWithRed_green_blue_alpha_(1, 1, 1, 1)).imageByCroppingToRect_(rect(0, 0, width, height));
      const result = moved.imageByCompositingOverImage_(white);
      result.retain();
      preparedImages.set(key, result);
      return result;
    }, replaceFrame = function(sampleBuffer) {
      ...
      if (frameCount === 1 || frameCount % 30 === 0) {
        send({ type: "frame-replaced", count: frameCount, width, height });
      }
    }, hookDelegateClass = function(className) {
      if (!className || hookedClasses.has(className)) return;
      const klass = frida_objc_bridge_default.classes[className];
      if (!klass || !klass[selector2]) return;
      const method2 = klass[selector2];
      const implementationKey = method2.implementation.toString();
      if (!hookedImplementations.has(implementationKey)) {
        try {
          Interceptor.attach(method2.implementation, {
            onEnter(args) {
              try {
                const outputObject = new frida_objc_bridge_default.Object(args[2]);
                const captureDelegate = new frida_objc_bridge_default.Object(args[0]);
                if (outputObject.isKindOfClass_(frida_objc_bridge_default.classes.AVCaptureVideoDataOutput)) {
                ...
                if (enabled && qrImage !== null && outputObject.isKindOfClass_(frida_objc_bridge_default.classes.AVCaptureVideoDataOutput)) {
                  if (!pipelineReported) {
          ...
          send({ type: "hook-skipped", className, message: String(error) });
      ...
      hookedClasses.add(className);
      send({ type: "hook-installed", className });
    }, hookMetadataController = function() {
      const method2 = frida_objc_bridge_default.classes.WACameraController["- captureOutput:didOutputMetadataObjects:fromConnection:"];
      if (!method2) return;
      Interceptor.attach(method2.implementation, {
        onEnter(args) {
          try {
            const controller = new frida_objc_bridge_default.Object(args[0]);
            rememberCameraController(controller);
            rememberMetadataPipeline(args[2], args[4]);
            send({
              type: "metadata-pipeline-captured",
              controllerClass: controller.$className,
              outputClass: args[2].isNull() ? null : new frida_objc_bridge_default.Object(args[2]).$className
            });
          } catch (error) {
            send({ type: "metadata-output-warning", message: String(error) });
          }
    ...
    }, hookFutureDelegates = function() {
      const method2 = frida_objc_bridge_default.classes.AVCaptureVideoDataOutput["- setSampleBufferDelegate:queue:"];
      Interceptor.attach(method2.implementation, {
        onEnter(args) {
          const delegate = args[2];
          if (!delegate.isNull()) {
            hookDelegateClass(new frida_objc_bridge_default.Object(delegate).$className);
          }
        }
      });
    }, hookScannerAcceptance = function() {
      const scanner = frida_objc_bridge_default.classes.WAWebClientQRCodeScannerViewController;
      [
        ["- willAcceptQRCode", "qr-will-accept"],
        ["- didAcceptQRCode", "qr-accepted"]
      ].forEach(function(entry) {
        const method2 = scanner[entry[0]];
        if (!method2) return;
        Interceptor.attach(method2.implementation, {
          onEnter() {
            send({ type: entry[1] });
    ...
    }, scanExistingDelegates = function() {
      const modules = frida_objc_bridge_default.enumerateLoadedClassesSync();
      Object.keys(modules).forEach(function(moduleName) {
        modules[moduleName].forEach(function(className) {
          const klass = frida_objc_bridge_default.classes[className];
          if (klass && klass.$ownMethods.indexOf(selector2) !== -1) {
      ...
      recv("set-qr", function(message, data) {
        try {
          ...
          const nsData = frida_objc_bridge_default.classes.NSData.dataWithBytes_length_(storage, bytes.byteLength);
          const uiImage = frida_objc_bridge_default.classes.UIImage.imageWithData_(nsData);
          if (uiImage === null) throw new Error("PNG \u89E3\u7801\u5931\u8D25");
          clearPreparedImages();
          if (qrImage !== null) qrImage.release();
          qrImage = frida_objc_bridge_default.classes.CIImage.imageWithCGImage_(uiImage.CGImage());
          qrImage.retain();
          releaseQrCodeObject();
          ...
          metadataPlaceholder = frida_objc_bridge_default.classes.AVMetadataMachineReadableCodeObject.alloc().init();
    ...
    const hookedClasses = /* @__PURE__ */ new Set();
    const hookedImplementations = /* @__PURE__ */ new Set();
    const selector2 = "- captureOutput:didOutputSampleBuffer:fromConnection:";
    ...
    const ciContext = frida_objc_bridge_default.classes.CIContext.contextWithOptions_(NULL);
    ciContext.retain();
    const colorSpace = CGColorSpaceCreateDeviceRGB();
    const qrFactoryMethod = frida_objc_bridge_default.classes.WACameraController["- qrCodeObjectWithMetadataObject:"];
