...
`;
  ...
  // daemon_camera_inventory_agent.js
  var moduleFilter = /(CMCapture|AVFCapture|H13ISP|appleh13camerad|mediaserverd)/i;
  var classFilter = /(capture|camera|stream|remote|client|source|sink|output|connection)/i;
  var methodFilter = /(sample|buffer|surface|frame|output|stream|client|connection|enqueue|dequeue|send|deliver|consume|produce)/i;
  var loaded = frida_objc_bridge_default.enumerateLoadedClassesSync();
  var inventory = [];
  Object.keys(loaded).forEach((modulePath) => {
    ...
    loaded[modulePath].forEach((className) => {
      ...
      if (!klass) return;
      const methods = klass.$ownMethods.filter((method2) => methodFilter.test(method2));
