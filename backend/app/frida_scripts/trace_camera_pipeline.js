...
`;
  ...
  // trace_camera_pipeline_agent.js
  var targetClasses = [
  ...
  function interesting(method2) {
    return /(capture|sample|buffer|video|frame|output|producer|consumer|session|metadata|pixel|image|qr|scan|detect|code)/i.test(method2);
  }
  ...
  function inspectClass(className) {
    ...
    const instances = frida_objc_bridge_default.chooseSync(klass).map((object) => {
      const ivars = {};
