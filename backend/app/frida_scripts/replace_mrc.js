...
`;
  ...
  function Runtime() {
    ...
    this.implement = function(method3, fn) {
      return new NativeCallback(fn, method3.returnType, method3.argumentTypes);
    };
    ...
    function ObjCObject(handle2, protocol, cachedIsClass, superSpecifier2) {
      ...
      function findMethod(rawName) {
        let method3 = cachedMethods.get(rawName);
        if (method3 !== void 0)
          return method3;
        const tokens = parseMethodName(rawName);
        const fullName = tokens[2];
        method3 = cachedMethods.get(fullName);
        if (method3 !== void 0) {
          cachedMethods.set(rawName, method3);
          return method3;
        }
        const kind = tokens[0];
        const name = tokens[1];
        ...
        cachedMethods.set(fullName, method3);
        cachedMethods.set(rawName, method3);
        if (kind === defaultKind)
          cachedMethods.set(jsMethodName(name), method3);
        return method3;
      }
      function stealTypesFromProtocols(klass, fullName) {
        ...
        const method3 = candidates[fullName];
        if (method3 === void 0) {
          return null;
        }
        return method3.types;
      }
        ...
        if (cachedProtocolMethods === null) {
          ...
          Object.keys(protocols).forEach(function(name) {
            ...
            Object.keys(m2).forEach(function(fullMethodName) {
              const method3 = m2[fullMethodName];
      ...
      function findMethodWrapper(name) {
        const method3 = findMethod(name);
        if (method3 === null)
          return null;
        let wrapper = method3.wrapper;
        if (wrapper === null) {
          wrapper = makeMethodInvocationWrapper(method3, self, superSpecifier2, defaultInvocationOptions);
          method3.wrapper = wrapper;
        }
        return wrapper;
      }
    ...
    function registerClass(properties) {
      ...
      try {
        ...
        Object.keys(methods).forEach(function(rawMethodName) {
          ...
          if (typeof value === "function") {
            ...
            if (rawMethodName in superClass) {
              ...
              for (let protocol of protocols) {
                const method4 = protocol.methods[rawMethodName];
                if (method4 !== void 0) {
                  types3 = method4.types;
                  break;
                }
              }
            }
          ...
          let types2 = method3.types;
          if (types2 === void 0) {
            types2 = unparseSignature(method3.retType, [kind === "+" ? "class" : "object", "selector"].concat(method3.argTypes));
          }
          const signature2 = parseSignature(types2);
          const implementation2 = new NativeCallback(
            makeMethodImplementationWrapper(signature2, method3.implementation),
            signature2.retType.type,
            signature2.argTypes.map(function(arg) {
              return arg.type;
            })
    ...
    function registerProtocol(properties) {
      ...
      const methodSpecs = Object.keys(methods).map(function(rawMethodName) {
        const method3 = methods[rawMethodName];
        const match = /([+\-])\s(\S+)/.exec(rawMethodName);
        if (match === null)
          throw new Error("Invalid method name");
        const kind = match[1];
  ...
  // replace_mrc_agent.js
  var getImageBuffer = new NativeFunction(
    Module.getGlobalExportByName("CMSampleBufferGetImageBuffer"),
    "pointer",
    ["pointer"]
  );
  var getWidth = new NativeFunction(Module.getGlobalExportByName("CVPixelBufferGetWidth"), "ulong", ["pointer"]);
  var getHeight = new NativeFunction(Module.getGlobalExportByName("CVPixelBufferGetHeight"), "ulong", ["pointer"]);
  var getPixelFormat = new NativeFunction(Module.getGlobalExportByName("CVPixelBufferGetPixelFormatType"), "uint", ["pointer"]);
  var createColorSpace = new NativeFunction(Module.getGlobalExportByName("CGColorSpaceCreateDeviceRGB"), "pointer", []);
  var ciImageClass = frida_objc_bridge_default.classes.CIImage;
  var ciColorClass = frida_objc_bridge_default.classes.CIColor;
  var ciContext = frida_objc_bridge_default.classes.CIContext.contextWithOptions_(NULL);
  ciContext.retain();
  var colorSpace = createColorSpace();
  ...
  function preparedImage(width, height) {
    ...
    const background = ciImageClass.imageWithColor_(ciColorClass.colorWithRed_green_blue_alpha_(1, 1, 1, 1)).imageByCroppingToRect_(rect(0, 0, width, height));
    const image = foreground.imageByCompositingOverImage_(background);
    image.retain();
    prepared.set(key, image);
    return image;
  }
  var method2 = frida_objc_bridge_default.classes.BWMRCNode["- renderSampleBuffer:forInput:"];
  var listener = Interceptor.attach(method2.implementation, {
    onEnter(args) {
