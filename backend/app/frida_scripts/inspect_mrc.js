...
`;
  ...
  // inspect_mrc_agent.js
  var classNames = ["BWMRCNode", "BWPixelTransferNode"];
  var counts = {};
  var listeners = [];
  ...
  classNames.forEach((className) => {
    ...
    if (!klass) return;
    classes[className] = { methods: klass.$ownMethods, instances: frida_objc_bridge_default.chooseSync(klass).map(inspect) };
    klass.$ownMethods.filter((method2) => /(render|sample|buffer|detect|process|code|metadata)/i.test(method2)).forEach((selector2) => {
      const key = `${className} ${selector2}`;
      counts[key] = 0;
      const method2 = klass[selector2];
      if (!method2) return;
      listeners.push(Interceptor.attach(method2.implementation, {
        onEnter() {
          counts[key] += 1;
        }
      }));
