...
`;
  ...
  recv("decode", function(_message, data) {
    try {
      ...
      const image = frida_objc_bridge_default.classes.CIImage.imageWithCGImage_(uiImage.CGImage());
      send({ type: "stage", name: "ciimage", extent: image.extent() });
      const options = frida_objc_bridge_default.classes.NSDictionary.dictionaryWithObject_forKey_(
        nsString("CIDetectorAccuracyHigh"),
        nsString("CIDetectorAccuracy")
      );
      const detector = frida_objc_bridge_default.classes.CIDetector.detectorOfType_context_options_(
