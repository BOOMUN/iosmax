...
`;
  ...
  function compileModule() {
    ...
    return {
      handle: cm,
      choose(klass2, considerSubclasses) {
        const result = [];
        const countPtr = Memory.alloc(4);
        const matches = _choose(klass2, considerSubclasses ? 1 : 0, countPtr);
        try {
  ...
  function Runtime() {
        ...
        get(target, property, receiver) {
          switch (property) {
            ...
            case "valueOf":
              return valueOf;
            default:
              const klass2 = findClass(property);
              return klass2 !== null ? klass2 : void 0;
          }
        },
    ...
    function ObjCObject(handle2, protocol, cachedIsClass, superSpecifier2) {
      ...
      if (cachedIsClass === void 0) {
        const klass2 = api.object_getClass(handle2);
        const key = klass2.toString();
        if (!realizedClasses.has(key)) {
          api.objc_lookUpClass(api.class_getName(klass2));
          ...
          switch (property) {
            ...
            case "$methods":
              if (cachedNativeMethodNames === null) {
                const klass2 = superSpecifier2 ? superSpecifier2.add(pointerSize).readPointer() : classHandle();
                const meta = api.object_getClass(klass2);
            ...
            case "$ownMethods":
              if (cachedOwnMethodNames === null) {
                ...
                const instanceMethods = collectMethodNames(klass2, "- ");
      ...
      function stealTypesFromProtocols(klass2, fullName) {
        const candidates = Object.keys(klass2.$protocols).map((protocolName) => flatProtocolMethods({}, klass2.$protocols[protocolName])).reduce((allMethods, methods) => {
    ...
    function collectMethodNames(klass2, prefix) {
      const names = [];
      const numMethodsBuf = Memory.alloc(pointerSize);
      const methodHandles = api.class_copyMethodList(klass2, numMethodsBuf);
      try {
        const numMethods = numMethodsBuf.readUInt();
        for (let i = 0; i !== numMethods; i++) {
          const methodHandle = methodHandles.add(i * pointerSize).readPointer();
  ...
  // inspect_remote_sink_agent.js
  var klass = frida_objc_bridge_default.classes.BWRemoteQueueSinkNode;
  var selector2 = "- renderSampleBuffer:forInput:";
  var counts = {};
  ...
  function inspect(object) {
    ...
    Object.keys(object.$ivars).forEach((name) => {
      ...
      try {
        ivars[name] = summarize(object.$ivars[name]);
      } catch (error) {
  ...
  var listener = Interceptor.attach(klass[selector2].implementation, {
    onEnter(args) {
  ...
  setTimeout(() => {
    const instances = frida_objc_bridge_default.chooseSync(klass).map(inspect);
