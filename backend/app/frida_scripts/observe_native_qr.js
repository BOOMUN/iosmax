...
`;
  ...
  // observe_native_qr_agent.js
  function isNil(value) {
    return value === null || value === void 0 || value.handle && value.handle.isNull();
  }
  function inspectQrObject(object, metadata) {
    const result = { objectClass: object.$className };
    const code2 = typeof object.code === "function" ? object.code() : null;
    const data = typeof object.data === "function" ? object.data() : null;
    const metadataString = metadata && typeof metadata.stringValue === "function" ? metadata.stringValue() : null;
    result.codeClass = isNil(code2) ? null : code2.$className;
    result.codeLength = isNil(code2) ? 0 : code2.toString().length;
    ...
    result.symbolVersion = typeof object.symbolVersion === "function" ? Number(object.symbolVersion()) : null;
    ...
    if (!isNil(code2) && !isNil(metadataString) && typeof code2.isEqualToString_ === "function") {
      result.codeEqualsMetadataText = Boolean(code2.isEqualToString_(metadataString));
    }
  ...
  if (!frida_objc_bridge_default.available) {
    ...
    const factory = frida_objc_bridge_default.classes.WACameraController["- qrCodeObjectWithMetadataObject:"];
    const delivery = frida_objc_bridge_default.classes.WAWebClientQRCodeScannerViewController["- cameraController:didDetectQRCode:"];
    const metadataDelivery = frida_objc_bridge_default.classes.WACameraController["- captureOutput:didOutputMetadataObjects:fromConnection:"];
    Interceptor.attach(metadataDelivery.implementation, {
      onEnter(args) {
        try {
          const objects = new frida_objc_bridge_default.Object(args[3]);
    ...
    Interceptor.attach(factory.implementation, {
      ...
      onLeave(retval) {
        try {
          ...
          const object = new frida_objc_bridge_default.Object(retval);
          const metadata = this.metadataPointer.isNull() ? null : new frida_objc_bridge_default.Object(this.metadataPointer);
    ...
    Interceptor.attach(delivery.implementation, {
      onEnter(args) {
        try {
          const object = new frida_objc_bridge_default.Object(args[3]);
