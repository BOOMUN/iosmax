...
(() => {
  ...
  function getApi() {
    ...
    pending.forEach(function(api2) {
      ...
      const exportByName = (Process.findModuleByName(api2.module)?.enumerateExports() ?? []).reduce(function(result2, exp) {
...
`;
  ...
  function Runtime() {
    ...
    function ObjCIvars(instance, classHandle) {
      ...
      function toJSON() {
        return Object.keys(self).reduce(function(result2, name) {
          result2[name] = self[name];
          return result2;
        }, {});
      }
      function toString() {
  ...
  // inspect_bw_graph_agent.js
  var classNames = ["BWNodeInput", "BWNodeOutput", "BWNodeConnection", "BWRemoteQueueSinkNode"];
  var result = {};
  ...
  classNames.forEach((className) => {
    ...
    result[className] = {
      ...
      instances: frida_objc_bridge_default.chooseSync(klass).map(inspect)
    };
  });
